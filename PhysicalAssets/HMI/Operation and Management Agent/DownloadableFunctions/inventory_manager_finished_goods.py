...
def introduction():
    ...
    performative_types = 'request_finishedgoods_details'
    ...
    for root, dirs, files in os.walk("C:\\Users\\ashut\\OneDrive\\Desktop\\Custom Function Integration\\Machine_Twin 1", topdown=False):
        ...
        for name in files:
            if name == 'finishedgoods.xlsx':
                ...
                reply_dict["finishedgoods"] = []

                #reply_json[twinName].append(fileraw_val)
                fileraw_val = fileraw_val.reset_index()
                for index, row in fileraw_val.iterrows():

                    reply_dict["finishedgoods"].append({"job_id" : row["job-id"],"date": str(row["date"].to_pydatetime().date()), "time": str(row["time"])})
