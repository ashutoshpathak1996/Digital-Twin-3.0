...
def introduction():
    function_type = 'tool_path_optimizer'
    dependant_functions = []
...
def print_solution(manager, routing, solution,data,thickness,spindle_speed=1000,feed_rate=50):
    ...
    feed = feed_rate
    return CNC_code,holesNo,distance,feed

#filename = './media/30x30-5holes.stp'

#filename is actually filepath
def toolPathOptimization(filename):
...
##### For guided Local search enable this and disable above
    ...
    if solution:
        CNC_code=print_solution(manager, routing, solution,data,thickness)
    return CNC_code
